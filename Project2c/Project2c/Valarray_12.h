...
//template<typename T>struct decide;
...
//};
//
//template<typename T,typename unused1 =typename isvec_wrap<T>::value_type>
//struct decide{
//    using type = T;
//}
...
//    using vec_w = vec_wrap< proxy<on_off_decoration<T>,on_off_decoration<F>,op>>;
//    static constexpr bool  isok= isok1&&isok2;
//    using type =typename enableif<isok, vec_w>::type;
//};


//that's the return type of operator
//template<typename T,typename F>
//Bresult<T,F,std::plus<typename choose_type< typename T::value_type,typename F::value_type>::type>>operator+(const T& x,const F& y){
//    std::plus<typename choose_type<typename T::value_type,typename F::value_type>::type> f;
//    return Bresult<T,F,std::plus<typename choose_type< typename T::value_type,typename F::value_type>::type>>{(typename on_off_decoration<T>::value_type)x,(typename on_off_decoration<F>::value_type)y,f};
//}
